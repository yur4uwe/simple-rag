...
def run_demo(mode: str, question: str, use_rag: bool = True, model_name: str = "openrouter/free"):
    ...
    chain = get_chain(mode, model_name, api_key, use_rag=use_rag)  # pyright: ignore[reportUnknownVariableType]
    
    rag_label = "RAG" if use_rag else "NO-RAG"
    print(f"\n{'=' * 20}")
    print(f"MODE: {mode.upper()} ({rag_label})")
...
if __name__ == "__main__":
    ...
    run_demo("default", q1, use_rag=False)
    run_demo("default", q1, use_rag=True)

    print("\n\n--- DEMO 2: TECHNICAL COMPARISON ---")
    run_demo("coder", q2, use_rag=False)
    run_demo("coder", q2, use_rag=True)
